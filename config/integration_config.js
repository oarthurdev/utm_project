...
const config = {
  ...
  kommo: {
    ...
    // API endpoints
    baseUrl: process.env.KOMMO_ACCOUNT_DOMAIN
      ? `https://${process.env.KOMMO_ACCOUNT_DOMAIN}/api/v4`
      : null,
  ...
  n8n: {
    ...
    webhookPaths: {
      utmCapture: {
        mazi:
          process.env.N8N_WEBHOOK_PATH_UTM_CAPTURE_MAZI || "utm_capture_mazi",
        dicasa:
          process.env.N8N_WEBHOOK_PATH_UTM_CAPTURE_DICASA ||
          "utm_capture_dicasa",
      },
      facebookEvents: {
  ...
  logging: {
    ...
    files: {
      ...
      offlineEvents:
        process.env.LOG_FILE_OFFLINE_EVENTS || "logs/offline_events.log",
...
function validateConfig() {
  ...
  Object.entries(config.kommo.utmFields).forEach(([field, value]) => {
    if (!value) {
      warnings.push(
        `KOMMO_UTM_${field.toUpperCase()}_FIELD_ID is not configured`,
      );
    }
  });
  ...
  if (config.slack.botToken && !config.slack.channelId) {
    warnings.push(
      "SLACK_BOT_TOKEN is configured but SLACK_CHANNEL_ID is missing",
    );
  }
